...
function OrdersContent() {
  const { orders, updateOrder, updateOrderStatus, settings } = usePixelCart();
  const firestore = useFirestore();
  const searchParams = useSearchParams();
  const statusParam = searchParams.get('status');

  const [searchTerm, setSearchTerm] = useState('');
  // Default to 'pending' to show only new orders that need processing
  ...
  const statusOptions: { label: string; value: OrderStatus | 'all'; icon: any }[] = [
    ...
    { label: 'Au bureau', value: 'at_office', icon: MapPin },
    { label: 'Dispatch', value: 'dispatch', icon: Clipboard },
    { label: 'Expédié', value: 'shipped', icon: Truck },
  ...
  const getStatusBadge = (status: Order['status']) => {
    switch (status) {
      ...
      case 'shipped': return <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">Expédié</Badge>;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <Card className="border-none shadow-md bg-white">
          ...
          <CardContent>
            <div className="text-2xl font-black">{orders.filter(o => ['shipped', 'delivering'].includes(o.status)).length} Colis</div>
          </CardContent>
        </Card>
        <Card className="border-none shadow-md bg-white">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
