...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert integration engineer for BRICO PRO DZ.
...
If the API key is valid (minimum 20 characters), confirm that the order has been successfully mapped and is now visible in the "Pas encore expédiées" section. Return a mock tracking number starting with 'BP-'.
  ...
  async input => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          success: true,
          trackingNumber: data.tracking_number || data.id || `BP-${Date.now()}`,
